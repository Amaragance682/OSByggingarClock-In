...
class AdminApp(tk.Tk):
    ...
    def show_control_board(self):
        ...
        for company, count in sorted(summary.items()):
            row = tk.Frame(self.main_area, bg="#f4f4f4")
            row.pack(anchor="w", padx=30, pady=5)

            label = tk.Label(row, text=f"• {company}:", font=("Helvetica", 12, "bold"), bg="#f4f4f4")
            label.pack(side="left")

            value = tk.Label(row, text=f"{count} people", font=("Helvetica", 12), bg="#f4f4f4", fg="#007700")
        ...
        for user in self.users:
            ...
            for log in logs:
                if log.get("clock_out") is None:
                    ...
                    summary[company] = summary.get(company, 0) + 1
                    break  # Only count the person once, even if multiple open shifts
